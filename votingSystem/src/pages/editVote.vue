<template>
  <div class="editVote layout">
      ...
      <div class="content">
              <div  v-if="false" class="layout-left steps">
              ...
              <div class="center forms">
                    <el-collapse v-model="activeName"  v-loading="loading" accordion>
                        <el-collapse-item v-show="isEditVote" title="修改投票活动" name="1">
                           <div class="wrap">
                        ...
                        <el-collapse-item  v-show="isEditVoteItems"  title="修改投票项" name="2">
...
<script>
  ...
  export default {
    ...
    computed: {
      isEditVoteItems () {
        return this.$route.params.name === 'editVoteItems'
      },
      isEditVote () {
    ...
    mounted () {
      var dom = document.getElementsByClassName('editVote')[0]
      ...
      initeDate () {
        ...
        .then(function (res) {
          ...
          res.data.datas.voteItemDOs.forEach(item => {
            ...
            tempObj.resourceUrl = item.docUrl
            tempObj.key = item.itemIndex
            tempObj.imageUrl = item.imageUrl
            tempObj.docId = item.docId
            let count = that.voteItems.max - item.title.length
...
.editVote .voteItems .voteItemBlock{
  width: 95%;
  padding: 20px 10px;
  margin:10px auto;
  border-radius: 5px;
